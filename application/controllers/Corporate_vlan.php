	...
	public function get_vlan_cir(){
		if(isLoggedIn()){
			...
			foreach ($data->result() as $key => $value) {
				?>
				<tr style="<?php echo ($value->freeze == 1) ? 'background-color:#1a437a66;color:white;' : '';?>">
...
//
	...
	public function insert_vlan_service(){
		if(isLoggedIn()){
			...
			$connectedto = $this->input->post('connectedto');
//
			$this->Model_Corp_VLAN->insert_vlan_service($corporate,$subcorp,$location,$vlan_id,$pkg,$pripop,$secpop,$email,$poc,$longitude,$latitude,$connectedto);
...
//
	public function insert_vlan_p2p(){
		if(isLoggedIn()){
			...
			$connectedto = $this->input->post('connectedto');
//
			$this->Model_Corp_VLAN->insert_vlan_P2P($corporate,$subcorp,$location,$vlan_id,$pkg,$pripop,$secpop,$email,$poc,$longitude,$latitude,$connectedto);
			echo 'Success : Successfully Added';
//
		}else{
			redirect(base_url('login'));
...
//
	public function insert_vlan_id(){
		if(isLoggedIn()){
			...
			$connectedto = $this->input->post('connectedto');
//
			$this->Model_Corp_VLAN->insert_vlan_ID($id,$corporate,$location,$vlan_id,$pkg,$pripop,$secpop,$email,$poc,$longitude,$latitude,$connectedto);
			echo 'Success : Successfully Added';
//
		}else{
			redirect(base_url('login'));
...
// //
				$this->Model_Corp_VLAN->insert_vlan($ipRange,$subnet,$diff,$corporate,$area,$vlan_name,$vlan_id,$pkg,$bandwidth,$pripop,$secpop,$email,$poc,$longitude,$latitude,$connectedto);
				...
				<div class="row">
					...
					<div class="col-md-12 ">
						<div class="form-group">
							...
							<input type="text" class="form-control" name="connectedto" id="exampleFormControlInput1" required="" value="<?= $data->connected_to;?>">
						</div>
					</div>
					<div class="col-md-12">
						<label for="exampleFormControlInput1">Freeze </label>
						<label class="switch"> 
							<input type="hidden" name="freeze" value="0">
							<input type="checkbox" data-size="mini" value="1" name="freeze" <?php echo ($data->freeze == 1) ? 'checked' : ''; ?> >
...
//
		$data = array('corporate' => $corporate,'area' => $area, 'vlan_name' => $vlan_name , 'pkg' => $pkg ,'bandwidth' => $bandwidth , 'prim_pop' => $pripop, 'sec_pop' => $secpop ,'last_update' => date('Y-m-d H:i:s'), 'last_update_by' => $this->session->userdata['username'] ,'longitude' => $longitude, 'latitude' => $latitude, 'connected_to' => $connectedto, 'freeze' => $freeze);
//
...
//
	...
	public function update_ID(){
		if(isLoggedIn()){
			...
			$freeze = $this->input->post('freeze');
//
			$data = array('name' => $corporate, 'location' => $location, 'vlan_id' => $vlan_id, 'pkg' => $pkg, 'prim_pop' => $pripop, 'sec_pop' => $secpop, 'email' => $email, 'poc' => $poc ,'last_update' => date('Y-m-d H:i:s'),'last_update_by' => $this->session->userdata('username'),'longitude' => $longitude, 'latitude' => $latitude, 'connected_to' => $connectedto, 'freeze' => $freeze);
			//
...
//
	...
	public function update_P2P(){
		if(isLoggedIn()){
			...
			$freeze = $this->input->post('freeze');
//
			$data = array('sub_corp' => $subcorp, 'location' => $location, 'vlan_id' => $vlan_id, 'pkg' => $pkg, 'prim_pop' => $pripop, 'sec_pop' => $secpop, 'email' => $email, 'poc' => $poc ,'last_update' => date('Y-m-d H:i:s'),'last_update_by' => $this->session->userdata('username'),'longitude' => $longitude, 'latitude' => $latitude, 'connected_to' => $connectedto, 'freeze' => $freeze);
			$this->db->where('serial',$ser);
...
//
	...
	public function update_service(){
		if(isLoggedIn()){
			...
			$freeze = $this->input->post('freeze');
//
			$data = array('sub_corp' => $subcorp, 'location' => $location, 'vlan_id' => $vlan_id, 'prim_pop' => $pripop, 'sec_pop' => $secpop, 'email' => $email, 'poc' => $poc ,'last_update' => date('Y-m-d H:i:s'),'last_update_by' => $this->session->userdata('username'),'longitude' => $longitude, 'latitude' => $latitude, 'connected_to' => $connectedto,'freeze' => $freeze );
			$this->db->where('serial',$ser);
...
//
	...
	public function free_ips(){	
		$this->load->view('vlan_corporate_free_ip');
	}
	//
	public function get_free_ips(){
		$array = $this->Model_Corp_VLAN->showIP()->result_array();
		$array = array_column($array, "ip_address");
		//
		$sql = "SELECT DISTINCT (SUBSTRING_INDEX(ip_address,'.',3)) as ips from nms_corp_vlan";
