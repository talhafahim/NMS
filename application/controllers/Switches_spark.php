...
class Switches_spark extends CI_Controller {
	//
	public function __construct(){

		parent::__construct();
		//
		$this->load->model('Model_Switch_Spark');
	...
	{
		if(isLoggedIn() && in_array("switchspark", access_subCat()) )  {
			$data['pops'] = $this->Model_Corp_VLAN->show_pops();

			$data['cities'] = $this->Model_City->show_all_cities();
			$data['dealer'] = $this->Model_Switch_Spark->dealer_info('spark');
			$this->load->view('switch_spark',$data);
		}else {
	...
	{
		if(isLoggedIn() && in_array("SwitchSpark", access_cat()) ){
			$switid = $this->uri->segment(3);
			$data['data'] = $this->Model_Switch_Spark->show_switches(null,$switid)->row();
			...
			$data['ping'] = $this->Model_Switch_Spark->get_ping($data['data']->ip_address)->row();
			$data['city'] = $this->Model_City->showcitiesById($data['data']->city_id);
			// print_r($data['city']);
			// die();
			$this->load->view('view_switch_spark',$data);
		}else {
			redirect(base_url('login'));
		}
	}
	...
	public function get_switches(){
		if(isLoggedIn()){
			$data = $this->Model_Switch_Spark->show_switches();
			...
			foreach ($data->result() as $key => $value) {
				...
				<tr>
					...
					<?php
					$ping_resp = $this->Model_Switch_Spark->get_ping($value->ip_address)->row();
					...
					<td>
						<div class="btn-group">
							<a class="btn waves-effect waves-light btn-xs btn-success" href="<?= base_url();?>switches_spark/view/<?= $value->swit_id;?>"><i class="fa fa-eye"></i></a>
							<?php if(access_crud('SwitchSpark','update')){?>
							...
							<?php } if(access_crud('SwitchSpark','delete')){?>
	...
	public function insert_switch(){
		if(isLoggedIn()){
			...
			$city_id = $this->input->post('city_name');
				//
			$countIP = $this->Model_Switch_Spark->show_switches($ip,null,null,null,$city_id);
			if($countIP->num_rows() > 0 ){
				$error = 'Error : IP already exist at same city';
			}
			$iparray = explode('.', $ip);
			...
			//
			if($vlan == 3 && (!empty($iparray[0])) && (!empty($iparray[1])) && (!empty($iparray[2])) && (!empty($iparray[3])) && $countIP->num_rows() <= 0  ){
				//
				$error = null;
			}
			//
			...
			if($error == null){
				//
				$this->Model_Switch_Spark->insert_switch($switch,$ip,$dealer,$area,$version,$model,$pripop,$secpop,$vlan,$connect_to,$stack,$ether,$city_id);
	...
	public function delete_switch(){
		if(isLoggedIn() && (access_crud('switch','delete')) )  {
			...
			$this->db->delete('nms_switch_spark');
		//
	...
	public function update_form(){
		$ip = $this->input->post('ip');
		$data = $this->Model_Switch_Spark->show_switches($ip)->row();
		$pops = $this->Model_Corp_VLAN->show_pops();
		$dealer = $this->Model_Switch_Spark->dealer_info();
		$cities = $this->Model_City->show_all_cities();
		// print_r($pops->result());
		// die();
		// $city = $this->Model_City->showcitiesById($pops->city_id);
		?>
						...
						<div class="form-group">
							...
							<select class="form-control" name="pripop" id="exampleFormControlSelect1" required="">
								<?php if(!empty($data->prim_pop)){?>
									<option value="<?= $data->prim_pop; ?>"><?= $this->Model_Corp_VLAN->show_pops(null,$data->prim_pop)->row()->pop_name.' ('.$this->Model_Corp_VLAN->show_pops(null,$data->prim_pop)->row()->city_name.')'; ?></option>
								<?php }else{?>
						...
						<div class="form-group">
							...
							<select class="form-control" name="secpop" id="exampleFormControlSelect1" required="">
								<?php if(!empty($data->sec_pop)){?>
									<option value="<?= $data->sec_pop; ?>"><?= $this->Model_Corp_VLAN->show_pops(null,$data->sec_pop)->row()->pop_name.' ('.$this->Model_Corp_VLAN->show_pops(null,$data->prim_pop)->row()->city_name.')'; ?></option>
								<?php }else{?>
									<option value="">select pop</option>
								<?php } foreach($pops->result() as $value){?>
									<option value="<?php echo $value->pop_id;?>"><?php echo $value->pop_name.' ('.$value->city_name.')';?></option>
	...
	public function update_switch(){
		...
		$this->db->update('nms_switch_spark',$data);
			//
	...
	public function free_ips(){
		if(isLoggedIn() && in_array("free ips", access_subCat()) )  {
			$this->load->view('switch_spark_free_ip');
		}else{
			redirect(base_url('login'));
		}
	}
	...
	public function get_free_ip(){
		...
		//
		$sql = "SELECT DISTINCT(SUBSTRING_INDEX(SUBSTRING_INDEX(ip_address,'.',-2),'.',1)) as ips from nms_switch_spark";
